...
const urlParamssemhas = new URLSearchParams(window.location.search);
const selectedNPMsemhas = urlParamssemhas.get("NPM");

document.addEventListener("DOMContentLoaded", async () => {
  if (!selectedNPMsemhas) {
    console.log("Data tidak ditemukan!");
    window.location.href = `mahasiswa.html?NPM=${selectedNPMsemhas}`; // Redirect if no NPM
    return;
  }

  try {
    // Fetch Mahasiswa Data
    const mahasiswaResponse = await fetch("https://api.sheetbest.com/sheets/f4b8387c-6ddc-4485-b90b-6796d0b8fbf2/tabs/Semhas");
    const mahasiswaData = await mahasiswaResponse.json();
    const editData = mahasiswaData.find((item) => item.NPM === selectedNPMsemhas);

    if (!editData) {
      console.log("Data tidak ditemukan!");
      // window.location.href = `mahasiswa.html?NPM=${selectedNPMsemhas}`;
      return;
    } else {
      // date ===============================================================================
      ...
      document.getElementById("edit-tanggal-semhas").value = editData.Tanggal;
      document.getElementById("edit-npm-semhas").value = editData.NPM;
      document.getElementById("edit-nama-semhas").value = editData.Nama;
      document.getElementById("edit-bidang-semhas").value = editData.Bidang;
      document.getElementById("edit-judul-semhas").value = editData.Judul;
      document.getElementById("edit-dosbing1-semhas").value = editData.Pembimbing_1;
      document.getElementById("edit-dosbing2-semhas").value = editData.Pembimbing_2;

      document.getElementById("edit-dosji1-semhas").value = editData.Penguji_1;
      document.getElementById("edit-dosji2-semhas").value = editData.Penguji_2;

      document.getElementById("edit-jadwal-semhas").value = isoDate;
      document.getElementById("edit-jam-mulai-semhas").value = formattedJamMulai;
      document.getElementById("edit-jam-selesai-semhas").value = formattedJamSelesai;
      document.getElementById("edit-tempat-semhas").value = editData.Tempat;
      ...
      const bidangElement = document.getElementById("edit-bidang-semhas");
      const dosji1Dropdown = document.getElementById("edit-dosji1-semhas");
      const dosji2Dropdown = document.getElementById("edit-dosji2-semhas");
      const dosbing1Value = document.getElementById("edit-dosbing1-semhas").value; // Get the selected value of dosbing1
      const dosbing2Value = document.getElementById("edit-dosbing2-semhas").value; // Get the selected value of dosbing2
...
document.getElementById("konfirmasi-semhas").addEventListener("submit", async (e) => {
  e.preventDefault();
  ...
  try {
    const response = await fetch(`https://api.sheetbest.com/sheets/f4b8387c-6ddc-4485-b90b-6796d0b8fbf2/tabs/Semhas/NPM/${data.NPM}`, {
      method: "PUT",
      headers: {
...
function showToast(message, type, redirect = false) {
  ...
  if (redirect) {
    setTimeout(() => {
      window.location.href = `mahasiswa.html?NPM=${selectedNPMsemhas}`; // Redirect to profile.html
