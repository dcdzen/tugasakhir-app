...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const response = await fetch("https://api.sheetbest.com/sheets/506f8840-a871-4430-b4c5-ff4c7926af72/tabs/Sempro");
    if (!response.ok) {
  ...
  try {
    ...
    const dosbing1Dropdown = document.getElementById("dosji1");
    const dosbing2Dropdown = document.getElementById("dosji2");
    ...
    dosbing1Dropdown.innerHTML = '<option value="" disabled selected>Pilih Dosen</option>'; // Clear existing options
    ...
    if (filteredDosenForDosji1.length === 0) {
      dosbing1Dropdown.disabled = true;
      ...
      dosbing1Dropdown.appendChild(optgroup); // Add the optgroup to the dropdown
      dosbing1Dropdown.disabled = false;
    }

    // Group dosji2 options by Bidang using <optgroup>, excluding the selected Bidang
    dosbing2Dropdown.innerHTML = '<option value="" disabled selected>Pilih Dosen</option>'; // Clear existing options
    ...
    if (Object.keys(groupedDosenForDosji2).length === 0) {
      dosbing2Dropdown.disabled = true;
      ...
      Object.entries(groupedDosenForDosji2).forEach(([bidang, dosenList]) => {
        ...
        dosbing2Dropdown.appendChild(optgroup);
      });
      dosbing2Dropdown.disabled = false;
    }
  } catch (error) {
...
document.getElementById("add-data-form").addEventListener("submit", async (e) => {
  ...
  try {
    const getResponse = await fetch("https://api.sheetbest.com/sheets/506f8840-a871-4430-b4c5-ff4c7926af72/tabs/Semhas");
    if (!getResponse.ok) {
    ...
    const postResponse = await fetch("https://api.sheetbest.com/sheets/506f8840-a871-4430-b4c5-ff4c7926af72/tabs/Semhas", {
      method: "POST",
      headers: {
